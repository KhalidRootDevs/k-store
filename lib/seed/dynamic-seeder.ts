...
const PRODUCT_CONFIG = {
  smartphones: {
    ...
    features: ['5G', 'OLED Display', 'Fast Charging', 'Wireless Charging', 'Water Resistant', 'Face Recognition', 'Fingerprint Sensor'],
    colors: ['Black', 'White', 'Blue', 'Green', 'Purple', 'Red', 'Silver', 'Gold'],
    storage: ['64GB', '128GB', '256GB', '512GB', '1TB']
  },
  laptops: {
    brands: ['Apple', 'Dell', 'HP', 'Lenovo', 'Asus', 'Acer', 'MSI', 'Razer'],
    features: ['SSD Storage', 'Dedicated GPU', 'Thunderbolt', 'Backlit Keyboard', 'Touchscreen', '2-in-1', 'Gaming'],
    colors: ['Silver', 'Space Gray', 'Black', 'Blue', 'White'],
    processors: ['Intel i5', 'Intel i7', 'Intel i9', 'Ryzen 5', 'Ryzen 7', 'Ryzen 9', 'M1', 'M2', 'M3']
  },
  audio: {
    brands: ['Sony', 'Bose', 'JBL', 'Sennheiser', 'Audio-Technica', 'Beats', 'Samsung', 'Apple'],
    features: ['Noise Cancelling', 'Wireless', 'Water Resistant', 'Voice Assistant', 'Long Battery', 'Fast Charge'],
    colors: ['Black', 'White', 'Blue', 'Red', 'Silver', 'Pink']
  },
  gaming: {
    brands: ['Sony', 'Microsoft', 'Nintendo', 'Steam', 'Razer', 'Logitech', 'Corsair'],
    features: ['4K Gaming', 'VR Ready', 'Ray Tracing', 'High Refresh Rate', 'Customizable RGB', 'Wireless'],
    colors: ['Black', 'White', 'Special Edition']
  },
  general: {
    brands: ['Generic', 'Premium', 'Elite', 'Pro', 'Standard', 'Advanced'],
    features: ['Premium Quality', 'Eco Friendly', 'Durable', 'Lightweight', 'Compact', 'Multi-functional'],
...
export class DynamicSeeder {
  ...
  async seed(options: SeederOptions): Promise<SeederResult> {
    ...
    try {
      ...
      console.log(`🌱 Seeding ${options.categoriesCount} categories and ${options.productsCount} products...`);
  ...
  private async createCategories(count: number) {
    ...
    while (categories.length < count) {
      ...
      const category = await this.createCategory({
        ...
        description: faker.commerce.productDescription(),
        featured: faker.datatype.boolean({ probability: 0.2 }),
        parentId: categories.length > 0 && faker.datatype.boolean({ probability: 0.3 }) 
          ? faker.helpers.arrayElement(categories.filter(c => !c.parentId))._id 
          : null
      });
  ...
  private async createCategory(data: {
    ...
    const categoryData = {
      ...
      image: `/images/categories/${faker.helpers.slugify(data.name).toLowerCase()}.jpg`,
  ...
  private getRandomProductType(): keyof typeof PRODUCT_CONFIG {
    const types = Object.keys(PRODUCT_CONFIG) as (keyof typeof PRODUCT_CONFIG)[];
  ...
  private async createProduct(config: any, productType: string) {
    ...
    const productData = {
      ...
      tags: faker.helpers.arrayElements(
        [productType, brand.toLowerCase(), ...faker.helpers.arrayElements(config.features, 3)],
      ...
      seo: {
        ...
        keywords: faker.helpers.arrayElements([productType, brand, ...config.features], 5).join(', ')
      },
      images: Array.from({ length: faker.number.int({ min: 1, max: 4 }) }, (_, i) => 
        `/images/products/${faker.helpers.slugify(productName).toLowerCase()}-${i + 1}.jpg`
      )
    };
  ...
  private createVariants(count: number, config: any, basePrice: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      } while (usedCombinations.has(combinationKey) && usedCombinations.size < Object.keys(attributes).length * 5);

      usedCombinations.add(combinationKey);

      const variantPrice = basePrice * faker.number.float({ min: 0.8, max: 1.5 });
      ...
      variants.push({
        ...
        image: `/images/products/variant-${faker.string.alphanumeric(6).toLowerCase()}.jpg`
      });
    }
...
export async function seedDynamic(options: SeederOptions): Promise<SeederResult> {
