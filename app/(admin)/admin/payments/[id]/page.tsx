...
export default function PaymentDetailsPage() {
  ...
  const handleRefund = async () => {
    ...
    toast({
      ...
      description: "The refund has been successfully initiated and will be processed shortly.",
    })
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {payment.status === "Completed" && payment.amount > 0 && (
              <Button variant="outline" size="sm" onClick={handleRefund} disabled={isRefunding}>
                {isRefunding ? (
                  <>
        ...
        {payment.status === "Failed" && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {payment.failureReason || "This payment transaction failed. Please check the details below."}
            </AlertDescription>
          </Alert>
        )}
        ...
        {payment.status === "Refunded" && (
          <Alert>
            ...
            <AlertDescription>
              {payment.refundReason || "This payment has been refunded to the customer."}
            </AlertDescription>
          </Alert>
        )}
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        ${Math.abs(payment.amount).toFixed(2)} {payment.currency}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Processing Fee</span>
                      <span className="font-medium">${Math.abs(payment.fee).toFixed(2)}</span>
                    </div>
                    <Separator />
              ...
              <TabsContent value="timeline" className="mt-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Track the progress of this payment transaction</CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="text-sm font-mono">{payment.metadata.ipAddress}</div>
                            </div>
                            <div>
                          ...
                          <div>
                            ...
                            <div className="text-xs font-mono bg-muted p-2 rounded">{payment.metadata.userAgent}</div>
                          </div>
                        </>
                      )}
                      {payment.originalPaymentId && (
                        <div>
                          <div className="text-sm text-muted-foreground">Original Payment ID</div>
                          <div className="text-sm font-mono">{payment.originalPaymentId}</div>
                        </div>
                      )}
                    </div>
                  </CardContent>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">ID: {payment.customer.id}</div>
                    </div>
                  </div>
                  <Separator />
                  <div className="space-y-2">
                    <div className="text-sm">
                      <span className="font-medium">Email:</span> {payment.customer.email}
                    </div>
                    <div className="text-sm">
                      <span className="font-medium">Phone:</span> {payment.customer.phone}
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {payment.paypalDetails && (
                        <div className="text-sm text-muted-foreground">{payment.paypalDetails.email}</div>
                      )}
                    </div>
                  </div>
                  <Separator />
                  <div className="space-y-2">
                    {payment.cardDetails && (
                      <>
                        <div className="text-sm">
                          <span className="font-medium">Card Type:</span> {payment.cardDetails.type}
                        </div>
                        <div className="text-sm">
                        ...
                        <div className="text-sm">
                          <span className="font-medium">Country:</span> {payment.cardDetails.country}
                        </div>
                      </>
                    )}
                    {payment.paypalDetails && (
                      <>
                        <div className="text-sm">
                          <span className="font-medium">PayPal Email:</span> {payment.paypalDetails.email}
                        </div>
                        <div className="text-sm">
                          <span className="font-medium">Payer ID:</span> {payment.paypalDetails.payerId}
                        </div>
                      </>
                    )}
                  </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-1 text-sm">
                  ...
                  {payment.billingAddress.line2 && <div>{payment.billingAddress.line2}</div>}
                  <div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">View order details</div>
                    </div>
                  </div>
