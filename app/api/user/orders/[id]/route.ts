...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const normalizedOrder = {
      ...
      customer: {
        id: order.customer.id?._id ? order.customer.id._id.toString() : order.customer.id?.toString() || userId,
        name: order.customer.name || order.customer.id?.name || "Customer",
        email: order.customer.email || order.customer.id?.email || "unknown@example.com",
        phone: order.customer.phone || order.shippingAddress?.phone || "N/A",
        address: order.customer.address || order.shippingAddress?.address || "N/A",
      },
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { status, paymentStatus, trackingNumber, shippingMethod, notes, shippingAddress, billingAddress } = body
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (decoded.role !== "admin" && order.customer.id.toString() !== decoded.userId) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 403 })
    }
    ...
    order.timeline.push({
      status: "cancelled",
      date: new Date(),
      description: `Order cancelled by ${decoded.role === "admin" ? "admin" : "customer"}`,
